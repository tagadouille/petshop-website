<?php
require_once('annexe.php');
?>

<!DOCTYPE html>
<html>
<head>
        <meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link rel="stylesheet" type="text/css" href="style2.css">
        <link rel="icon" type="image/png" href="icon.png" />
    <title>Politique</title>
</head>
<body>

<?php 
navbar("none");
?>

<div id="connx">
<p>Bienvenue sur ChunChunMaru ! Nous sommes ravis de vous accueillir sur notre plateforme dédiée aux animaux et aux accessoires animaliers. Avant d'utiliser nos services, veuillez lire attentivement les présentes Conditions Générales d'Utilisation (CGU) qui régissent votre utilisation de notre site web et de nos services connexes. En utilisant ChunChunMaru, vous acceptez ces conditions dans leur intégralité. Si vous n'êtes pas d'accord avec une partie quelconque de ces conditions, veuillez ne pas utiliser notre site.</p>

<p>Acceptation des Conditions</p>

<p>1.1 En utilisant ChunChunMaru, vous reconnaissez avoir lu, compris et accepté les présentes CGU, ainsi que notre Politique de Confidentialité.</p>

<p>Utilisation du Site</p>

<p>2.1 Vous acceptez d'utiliser ChunChunMaru uniquement à des fins légales et conformes aux présentes CGU.</p>

<p>2.2 Vous êtes responsable de maintenir la confidentialité de votre compte et de vos informations de connexion, ainsi que de toutes les activités qui se produisent sous votre compte.</p>

<p>Contenu Utilisateur</p>

<p>3.1 Vous conservez la propriété intellectuelle de tout contenu que vous publiez sur ChunChunMaru. Cependant, en publiant du contenu sur notre site, vous nous accordez une licence mondiale, non-exclusive, transférable, sous-licenciable, gratuite pour utiliser, reproduire, distribuer, préparer des œuvres dérivées, afficher et exécuter ce contenu.</p>

<p>3.2 Vous acceptez de ne pas publier de contenu qui soit diffamatoire, obscène, offensant, violent, illégal ou qui enfreint les droits de propriété intellectuelle de tiers.</p>

<p>Responsabilités</p>

<p>4.1 ChunChunMaru s'efforce de fournir des services de qualité, mais nous ne garantissons pas que le site sera toujours sécurisé, sans erreur ou exempt de virus ou d'autres éléments nuisibles.</p>

<p>4.2 Nous ne sommes pas responsables des dommages directs, indirects, accidentels, spéciaux ou consécutifs découlant de votre utilisation de ChunChunMaru.</p>

<p>Modifications des Conditions</p>

<p>5.1 Nous nous réservons le droit de modifier les présentes CGU à tout moment. Toute modification sera effective dès sa publication sur le site.</p>

<p>Résiliation</p>

<p>6.1 Vous pouvez résilier votre compte sur ChunChunMaru à tout moment en nous contactant</p>

<p>6.2 Nous nous réservons le droit de suspendre ou de résilier votre accès à ChunChunMaru si nous estimons que vous avez enfreint les présentes CGU ou toute autre politique applicable.</p>

<p>Lois Applicables</p>

<p>7.1 Les présentes CGU sont régies et interprétées conformément aux lois en vigueur en France, sans égard aux principes de conflit de lois.</p>

<p>En utilisant ChunChunMaru, vous acceptez d'être lié par ces conditions. Si vous avez des questions ou des préoccupations concernant ces CGU, veuillez nous contacter à contact@monsiteanimaux.com. Merci de votre confiance et de votre utilisation de ChunChunMaru !</p>

</div>    

<?php
    footers();
?>

</body>
</html>